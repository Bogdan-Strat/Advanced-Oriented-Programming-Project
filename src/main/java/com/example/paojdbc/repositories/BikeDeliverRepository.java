...
@Repository
public class BikeDeliverRepository {
    @Value("${db.url}")
    private String url;
    @Value("${db.username}")
    ...
    public void addCarDeliver(BikeDeliver carDeliver){
    ...
    public List<BikeDeliver> getCarDelivers(){
        String sql = "select * from cardeliver";

        List<BikeDeliver> list = new ArrayList<>();
        ...
        try (Connection con = DriverManager.getConnection(url, username, password)) {
            ...
            while (res.next()) {
                BikeDeliver carDeliver = new BikeDeliver();
    ...
    public void deleteCarDeliver(BikeDeliver carDeliver){
    ...
    public void locationUpdate(BikeDeliver carDeliver, Integer newX, Integer newY){
